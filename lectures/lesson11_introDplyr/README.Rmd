---
title: "lesson 11: Data manipulation"
...
```{r}
beerDt %>% 
  filter(dplyr::between(Year, 1903, 1997))
...
`distinct` retains unique rows from the input table grouping by the variables passed to `...`. For instance, the combination of unique country-gender combinations:
...
By default, `distinct` retains only the columns of the grouping variables. Use `.keep_all = FALSE` to retain all columns:

1. Using `selfiesCasualties`, select distinct levels for `class` and keep all variables
```{r}
selfiesCasualties %>% 
...
2. Using `titanicDt`, arrange descending by `n` (check the examples for `desc()`)
```{r}
titanicDt %>% 
  arrange(desc(n))
```
...
`mutate()` adds a new variable. For example, we can add the birthdate of the victims by subtracting their age from the year of the accident:
...
1. In `beerDt`, convert the consumption from gallons per capita to liters (1 gallon 3.78 liters)
```{r}
beerDt %>%  mutate(LitersCapita = GallonsCapita *3.78)
```

2. In `socialNetwork_users` add a percent sign to the variable percent (use `paste0()`)
```{r}
...
```{r}
untidyReview_new <- untidyReview %>%  mutate(reviewStayYear = as.integer(reviewStayYear))
```


### Grouping

`group_by` adds grouping levels to a tibble. For instance, you can combine it with `summarise` to count the number of female and male victims by country:
...
# use ungroup() if you want to do a new summarise() on the preexisting group by
```

Note that every time you use `summarise()`, your tibble drops one grouping level. For instance in the latter example, data are still grouped by country, and any further dplyr verb would apply groupwise unless we `ungroup()` first.
...
```{r}
beerDt %>%  mutate(my_birthday = ifelse(Year < 1995, 'before_myBirthday', 'after_myBirthday')) %>% 
  group_by(my_birthday) %>%  summarise(average = mean(GallonsCapita))
```
...
`n()` is the count operator in dplyr (it works only within a `summarise()`, `mutate()` or `filter()`), and counts the rows by group level without taking any argument. Because counting observations is a very common task, dplyr offers the wrapper `count`, which is equivalent to `group_by %>% summarise( n() )` except that it `ungroup()` after. 
...
Functions from the `*_join` family serve to merge datasets. To join different datasets, you need at least one variable to serve as a key for joining. For instance, consider the case where you have two separates dataset about the count of twitter and FB users during the same years (Source: eMarketer):
...
If you do not pass a character vector of variables to join, `*_join` uses all the variables with common names across the two tables. 

In the previous example `full_join`, `left_join`, or `right_join` give the same result. However, each of them merge using different ruels. To show how the differe, we add a row to each dataset:
...
1. Use the appropriate join to add the column with full carrier names to `flights` using `airlines` :
