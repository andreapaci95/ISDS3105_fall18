...
ggplot() + geom_col(data=freqCasualties, aes(x=class, y=n, fill=gender))
```
...
ggplot(data = twitter_users, aes(Year,millions)) + 
  geom_line(color='red') + 
  geom_point()
```
...
ggplot() + geom_line(data = twitter_users, aes(Year, millions)) +
  geom_line(data= sn_users, aes(year, facebook), color = 'red')
```
...
```{r}
beer <- beerDt[beerDt$Year>1902,]
ggplot(data=beer, aes(Year, GallonsCapita)) + geom_line() +
  geom_point() 
```
